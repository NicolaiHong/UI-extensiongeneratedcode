    ...
    resolveConfig_default = (config) => {
      ...
      if (auth2) {
        headers.set(
          "Authorization",
          "Basic " + btoa(
            (auth2.username || "") + ":" + (auth2.password ? unescape(encodeURIComponent(auth2.password)) : "")
          )
        );
      }
  ...
  "node_modules/axios/lib/adapters/fetch.js"() {
    ...
    getFetch = (config) => {
      ...
      while (i--) {
        seed = seeds[i];
        target = map.get(seed);
        target === void 0 && map.set(seed, target = i ? /* @__PURE__ */ new Map() : factory(env4));
        map = target;
      }
      return target;
    };
...
function getServerUrl() {
  return vscode.workspace.getConfiguration("uigenai").get("serverUrl", "http://localhost:3000");
}
function initApiClient(opts) {
...
function getApi() {
  if (!_instance) {
    _instance = axios_default.create({
...
function rawPost(path2, body) {
  return axios_default.post(`${getServerUrl()}${path2}`, body, {
    headers: { "Content-Type": "application/json" },
    timeout: 3e4
  });
}
var vscode, _instance, _getToken, _onAuthFailed;
...
var unwrap, projectsApi;
var init_projects_api = __esm({
  "src/api/projects.api.ts"() {
    "use strict";
    init_client();
    unwrap = (r) => r.data?.data ?? r.data;
    projectsApi = {
      list: async () => unwrap(await getApi().get("/api/projects")),
      getById: async (id) => unwrap(await getApi().get(`/api/projects/${id}`)),
      create: async (data) => unwrap(await getApi().post("/api/projects", data)),
      update: async (id, data) => unwrap(await getApi().put(`/api/projects/${id}`, data)),
      delete: async (id) => {
...
// src/api/apis.api.ts
var unwrap4, apisApi;
var init_apis_api = __esm({
  "src/api/apis.api.ts"() {
    "use strict";
    init_client();
    unwrap4 = (r) => r.data?.data ?? r.data;
    apisApi = {
      list: async () => unwrap4(await getApi().get("/api/apis")),
      getById: async (id) => unwrap4(await getApi().get(`/api/apis/${id}`)),
      create: async (data) => unwrap4(await getApi().post("/api/apis", data)),
      update: async (id, data) => unwrap4(await getApi().put(`/api/apis/${id}`, data)),
      delete: async (id) => {
...
// src/api/apiConfigs.api.ts
var unwrap5, apiConfigsApi;
var init_apiConfigs_api = __esm({
  "src/api/apiConfigs.api.ts"() {
    "use strict";
    init_client();
    unwrap5 = (r) => r.data?.data ?? r.data;
    apiConfigsApi = {
      list: async (apiId) => unwrap5(await getApi().get(`/api/apis/${apiId}/configs`)),
      getById: async (apiId, id) => unwrap5(await getApi().get(`/api/apis/${apiId}/configs/${id}`)),
      create: async (apiId, data) => unwrap5(await getApi().post(`/api/apis/${apiId}/configs`, data)),
      update: async (apiId, id, data) => unwrap5(await getApi().put(`/api/apis/${apiId}/configs/${id}`, data)),
      delete: async (apiId, id) => {
...
// src/api/uiSchemas.api.ts
var unwrap6, uiSchemasApi;
var init_uiSchemas_api = __esm({
  "src/api/uiSchemas.api.ts"() {
    "use strict";
    init_client();
    unwrap6 = (r) => r.data?.data ?? r.data;
    uiSchemasApi = {
      list: async (apiId) => unwrap6(await getApi().get(`/api/apis/${apiId}/ui-schemas`)),
      getById: async (apiId, id) => unwrap6(await getApi().get(`/api/apis/${apiId}/ui-schemas/${id}`)),
      create: async (apiId, data) => unwrap6(await getApi().post(`/api/apis/${apiId}/ui-schemas`, data)),
      update: async (apiId, id, data) => unwrap6(await getApi().put(`/api/apis/${apiId}/ui-schemas/${id}`, data)),
      delete: async (apiId, id) => {
...
// src/api/generatedCodes.api.ts
var unwrap7, generatedCodesApi;
var init_generatedCodes_api = __esm({
  "src/api/generatedCodes.api.ts"() {
    "use strict";
    init_client();
    unwrap7 = (r) => r.data?.data ?? r.data;
    generatedCodesApi = {
      list: async (apiId) => unwrap7(await getApi().get(`/api/apis/${apiId}/generated-codes`)),
      getById: async (apiId, id) => unwrap7(await getApi().get(`/api/apis/${apiId}/generated-codes/${id}`)),
      create: async (apiId, data) => unwrap7(await getApi().post(`/api/apis/${apiId}/generated-codes`, data)),
      delete: async (apiId, id) => {
...
var unwrap8, deploymentsApi;
var init_deployments_api = __esm({
  "src/api/deployments.api.ts"() {
    "use strict";
    init_client();
    unwrap8 = (r) => r.data?.data ?? r.data;
    deploymentsApi = {
      list: async (apiId) => unwrap8(await getApi().get(`/api/apis/${apiId}/deployments`)),
      getById: async (apiId, id) => unwrap8(await getApi().get(`/api/apis/${apiId}/deployments/${id}`)),
      create: async (apiId, data) => unwrap8(await getApi().post(`/api/apis/${apiId}/deployments`, data)),
      update: async (apiId, id, data) => unwrap8(await getApi().put(`/api/apis/${apiId}/deployments/${id}`, data)),
      delete: async (apiId, id) => {
...
var projectCommands_exports = {};
__export(projectCommands_exports, {
  createProjectCmd: () => createProjectCmd,
  deleteProjectCmd: () => deleteProjectCmd,
  editProjectCmd: () => editProjectCmd
});
async function createProjectCmd() {
  const name = await vscode4.window.showInputBox({ title: "Create Project", prompt: "Project name", placeHolder: "My Awesome App" });
  if (!name) {
    return;
  }
  const desc = await vscode4.window.showInputBox({ title: "Create Project", prompt: "Description (optional)", placeHolder: "An AI-powered app" });
  try {
    const p = await projectsApi.create({ name, description: desc || void 0 });
    vscode4.window.showInformationMessage(`Project "${p.name}" created!`);
    vscode4.commands.executeCommand("uigenai.refreshSidebar");
  } catch (e) {
    vscode4.window.showErrorMessage(`Failed: ${e.response?.data?.error?.message || e.message}`);
  }
}
async function editProjectCmd(projectId) {
  const p = await projectsApi.getById(projectId);
  const name = await vscode4.window.showInputBox({ title: "Edit Project", value: p.name, prompt: "Project name" });
  if (!name) {
    return;
  }
  const desc = await vscode4.window.showInputBox({ title: "Edit Project", value: p.description || "", prompt: "Description" });
  try {
    await projectsApi.update(projectId, { name, description: desc || void 0 });
    vscode4.window.showInformationMessage(`Project updated!`);
    vscode4.commands.executeCommand("uigenai.refreshSidebar");
  } catch (e) {
    vscode4.window.showErrorMessage(`Failed: ${e.response?.data?.error?.message || e.message}`);
  }
}
async function deleteProjectCmd(projectId, projectName) {
  const confirm = await vscode4.window.showWarningMessage(`Delete project "${projectName}"?`, { modal: true }, "Delete");
  if (confirm !== "Delete") {
    return;
  }
  try {
    await projectsApi.delete(projectId);
    vscode4.window.showInformationMessage(`Project deleted.`);
    vscode4.commands.executeCommand("uigenai.refreshSidebar");
  } catch (e) {
    vscode4.window.showErrorMessage(`Failed: ${e.response?.data?.error?.message || e.message}`);
  }
...
// src/commands/apiCommands.ts
var apiCommands_exports = {};
__export(apiCommands_exports, {
  createApiCmd: () => createApiCmd,
  deleteApiCmd: () => deleteApiCmd,
  editApiCmd: () => editApiCmd
});
async function createApiCmd() {
  const name = await vscode5.window.showInputBox({ title: "Create API", prompt: "API name", placeHolder: "User Management API" });
  if (!name) {
    return;
  }
  const desc = await vscode5.window.showInputBox({ title: "Create API", prompt: "Description (optional)" });
  const baseUrl = await vscode5.window.showInputBox({ title: "Create API", prompt: "Base URL (optional)", placeHolder: "https://api.example.com/v1" });
  try {
    const a = await apisApi.create({ name, description: desc || void 0, base_url: baseUrl || void 0 });
    vscode5.window.showInformationMessage(`API "${a.name}" created!`);
    vscode5.commands.executeCommand("uigenai.refreshSidebar");
  } catch (e) {
    vscode5.window.showErrorMessage(`Failed: ${e.response?.data?.error?.message || e.message}`);
  }
}
async function editApiCmd(apiId) {
  const a = await apisApi.getById(apiId);
  const name = await vscode5.window.showInputBox({ title: "Edit API", value: a.name });
  if (!name) {
    return;
  }
  const desc = await vscode5.window.showInputBox({ title: "Edit API", value: a.description || "" });
  const baseUrl = await vscode5.window.showInputBox({ title: "Edit API", value: a.base_url || "" });
  try {
    await apisApi.update(apiId, { name, description: desc || void 0, base_url: baseUrl || void 0 });
    vscode5.window.showInformationMessage(`API updated!`);
    vscode5.commands.executeCommand("uigenai.refreshSidebar");
  } catch (e) {
    vscode5.window.showErrorMessage(`Failed: ${e.response?.data?.error?.message || e.message}`);
  }
}
async function deleteApiCmd(apiId, apiName) {
  const c = await vscode5.window.showWarningMessage(`Delete API "${apiName}"?`, { modal: true }, "Delete");
  if (c !== "Delete") {
    return;
  }
  try {
    await apisApi.delete(apiId);
    vscode5.window.showInformationMessage(`API deleted.`);
    vscode5.commands.executeCommand("uigenai.refreshSidebar");
  } catch (e) {
    vscode5.window.showErrorMessage(`Failed: ${e.response?.data?.error?.message || e.message}`);
  }
...
// src/commands/apiConfigCommands.ts
var apiConfigCommands_exports = {};
__export(apiConfigCommands_exports, {
  createApiConfigCmd: () => createApiConfigCmd,
  deleteApiConfigCmd: () => deleteApiConfigCmd,
  editApiConfigCmd: () => editApiConfigCmd
});
async function createApiConfigCmd(apiId) {
  ...
  const value = await vscode6.window.showInputBox({
    title: "Create Config",
    prompt: "Config value",
    placeHolder: "sk-...",
    password: false
  ...
  const secretPick = await vscode6.window.showQuickPick(
    [
  ...
  try {
    ...
    vscode6.window.showInformationMessage(`Config "${c.key}" created!`);
    vscode6.commands.executeCommand("uigenai.refreshSidebar");
  } catch (e) {
...
async function editApiConfigCmd(apiId, configId) {
  try {
    ...
    const value = await vscode6.window.showInputBox({
      ...
      prompt: cfg.is_secret ? "Enter new value (current is hidden)" : "Config value"
    });
    if (value === void 0) {
      return;
    }
    ...
    vscode6.window.showInformationMessage(`Config updated!`);
    vscode6.commands.executeCommand("uigenai.refreshSidebar");
  } catch (e) {
    vscode6.window.showErrorMessage(
      `Failed: ${e.response?.data?.error?.message || e.message}`
...
async function deleteApiConfigCmd(apiId, configId) {
  ...
  try {
    ...
    vscode6.window.showInformationMessage("Config deleted.");
    vscode6.commands.executeCommand("uigenai.refreshSidebar");
  } catch (e) {
    vscode6.window.showErrorMessage(
      `Failed: ${e.response?.data?.error?.message || e.message}`
...
var vscode6;
var init_apiConfigCommands = __esm({
  "src/commands/apiConfigCommands.ts"() {
    "use strict";
...
// src/commands/uiSchemaCommands.ts
var uiSchemaCommands_exports = {};
__export(uiSchemaCommands_exports, {
  createUiSchemaCmd: () => createUiSchemaCmd,
  deleteUiSchemaCmd: () => deleteUiSchemaCmd,
  editUiSchemaCmd: () => editUiSchemaCmd
});
async function createUiSchemaCmd(apiId) {
  ...
  const source = await vscode7.window.showQuickPick(
    [
      { label: "\u270F\uFE0F Paste JSON", value: "paste" },
      { label: "\u{1F4C4} From current editor", value: "editor" },
      { label: "\u{1F4C1} From file", value: "file" }
    ],
  ...
  if (source.value === "editor") {
    ...
    try {
      ...
      vscode7.window.showErrorMessage("Editor content is not valid JSON");
      return;
    }
    ...
    const buf = await vscode7.workspace.fs.readFile(files[0]);
    try {
      json = JSON.parse(Buffer.from(buf).toString("utf-8"));
    } catch {
      vscode7.window.showErrorMessage("File is not valid JSON");
      return;
    }
  } else {
    const raw = await vscode7.window.showInputBox({
      title: "Schema JSON",
    ...
    try {
      ...
      vscode7.window.showErrorMessage("Invalid JSON");
      return;
    }
  }
  try {
    const s = await uiSchemasApi.create(apiId, { name, schema_json: json });
    vscode7.window.showInformationMessage(`UI Schema "${s.name}" created!`);
    vscode7.commands.executeCommand("uigenai.refreshSidebar");
  } catch (e) {
...
async function editUiSchemaCmd(apiId, schemaId) {
  try {
    ...
    const saved = await vscode7.window.showInformationMessage(
    ...
    try {
      ...
      vscode7.window.showErrorMessage("Document is not valid JSON");
      return;
    }
    await uiSchemasApi.update(apiId, schemaId, { name, schema_json: json });
    vscode7.window.showInformationMessage(`UI Schema updated!`);
    vscode7.commands.executeCommand("uigenai.refreshSidebar");
  } catch (e) {
    vscode7.window.showErrorMessage(
      `Failed: ${e.response?.data?.error?.message || e.message}`
...
async function deleteUiSchemaCmd(apiId, schemaId) {
  ...
  try {
    ...
    vscode7.window.showInformationMessage("UI Schema deleted.");
    vscode7.commands.executeCommand("uigenai.refreshSidebar");
  } catch (e) {
    vscode7.window.showErrorMessage(
      `Failed: ${e.response?.data?.error?.message || e.message}`
...
var vscode7;
var init_uiSchemaCommands = __esm({
  "src/commands/uiSchemaCommands.ts"() {
    "use strict";
...
// src/commands/generatedCodeCommands.ts
var generatedCodeCommands_exports = {};
__export(generatedCodeCommands_exports, {
  applyGeneratedCodeCmd: () => applyGeneratedCodeCmd,
  deleteGeneratedCodeCmd: () => deleteGeneratedCodeCmd,
  viewGeneratedCodeCmd: () => viewGeneratedCodeCmd
});
async function viewGeneratedCodeCmd(apiId, codeId) {
  try {
    const code = await generatedCodesApi.getById(apiId, codeId);
    const doc = await vscode8.workspace.openTextDocument({ content: code.content, language: code.language || "typescript" });
    await vscode8.window.showTextDocument(doc, { preview: true });
  } catch (e) {
    vscode8.window.showErrorMessage(`Failed: ${e.response?.data?.error?.message || e.message}`);
  }
}
async function applyGeneratedCodeCmd(apiId, codeId) {
  try {
    const code = await generatedCodesApi.getById(apiId, codeId);
    const wf = vscode8.workspace.workspaceFolders;
    if (!wf?.length) {
      vscode8.window.showErrorMessage("Open a workspace first");
      return;
    }
    const uri = vscode8.Uri.joinPath(wf[0].uri, code.file_path.replace(/^\/+/, ""));
    try {
      await vscode8.workspace.fs.createDirectory(vscode8.Uri.joinPath(uri, ".."));
    } catch {
    }
    await vscode8.workspace.fs.writeFile(uri, Buffer.from(code.content, "utf-8"));
    vscode8.window.showInformationMessage(`Applied: ${code.file_path}`);
...
async function deleteGeneratedCodeCmd(apiId, codeId) {
  const c = await vscode8.window.showWarningMessage("Delete this generated code?", { modal: true }, "Delete");
  if (c !== "Delete") {
    return;
  }
  try {
    await generatedCodesApi.delete(apiId, codeId);
    vscode8.window.showInformationMessage("Code deleted.");
    vscode8.commands.executeCommand("uigenai.refreshSidebar");
  } catch (e) {
    vscode8.window.showErrorMessage(`Failed: ${e.response?.data?.error?.message || e.message}`);
  }
}
var vscode8;
var init_generatedCodeCommands = __esm({
  "src/commands/generatedCodeCommands.ts"() {
    "use strict";
    vscode8 = __toESM(require("vscode"));
    init_generatedCodes_api();
  }
});

// src/commands/deploymentCommands.ts
var deploymentCommands_exports = {};
__export(deploymentCommands_exports, {
  createDeploymentCmd: () => createDeploymentCmd,
  updateDeploymentStatusCmd: () => updateDeploymentStatusCmd
});
async function createDeploymentCmd() {
  const apis = await apisApi.list();
  if (!apis.length) {
    vscode9.window.showWarningMessage("No APIs found. Create an API first.");
    return;
  }
  const apiPick = await vscode9.window.showQuickPick(
  ...
  const env4 = await vscode9.window.showQuickPick(
  ...
  const provider = await vscode9.window.showInputBox({ title: "Provider (optional)", placeHolder: "vercel, netlify, etc." });
  try {
    const d = await deploymentsApi.create(apiPick.value, {
      environment: env4.value,
      provider: provider || void 0
    });
    vscode9.window.showInformationMessage(`Deployment created! Status: ${d.status}`);
    vscode9.commands.executeCommand("uigenai.refreshSidebar");
  } catch (e) {
    vscode9.window.showErrorMessage(`Failed: ${e.response?.data?.error?.message || e.message}`);
  }
}
async function updateDeploymentStatusCmd(apiId, deploymentId) {
  const status = await vscode9.window.showQuickPick(
  ...
  try {
    await deploymentsApi.update(apiId, deploymentId, { status: status.value });
    vscode9.window.showInformationMessage(`Deployment status updated to ${status.value}`);
    vscode9.commands.executeCommand("uigenai.refreshSidebar");
  } catch (e) {
    vscode9.window.showErrorMessage(`Failed: ${e.response?.data?.error?.message || e.message}`);
  }
...
var vscode9;
var init_deploymentCommands = __esm({
  "src/commands/deploymentCommands.ts"() {
    "use strict";
    vscode9 = __toESM(require("vscode"));
    init_deployments_api();
    init_apis_api();
  }
});
...
init_client();
var authApi = {
  login: async (emailOrUsername, password) => (await rawPost("/auth/login", { emailOrUsername, password })).data,
  register: async (email, password, displayName) => (await rawPost("/auth/register", { email, password, displayName })).data,
  refresh: async (refreshToken) => (await rawPost("/auth/refresh", { refreshToken })).data,
  logout: async (refreshToken) => {
    await rawPost("/auth/logout", { refreshToken });
  },
  getMe: async () => (await getApi().get("/auth/me")).data
};

// src/auth/authManager.ts
init_client();
var AT_KEY = "uigenai.accessToken";
var RT_KEY = "uigenai.refreshToken";
var USER_KEY = "uigenai.user";
var AuthManager = class {
  constructor(secrets, state) {
  ...
  async init() {
    ...
    const raw = this.state.get(USER_KEY);
    if (at && raw) {
      try {
  ...
  async logout() {
    ...
    await this.state.update(USER_KEY, void 0);
    this._user = null;
    this._onDidChange.fire(null);
  }
  async save(user, tokens) {
    this._user = user;
    await this.secrets.store(AT_KEY, tokens.accessToken);
    await this.secrets.store(RT_KEY, tokens.refreshToken);
    await this.state.update(USER_KEY, JSON.stringify(user));
  ...
  onAuthFailed() {
    this.logout();
    vscode2.window.showWarningMessage("Session expired. Please login again.", "Login").then((a) => {
      if (a === "Login") {
        vscode2.commands.executeCommand("uigenai.login");
      }
    });
  }
  dispose() {
...
// src/auth/loginWebview.ts
var vscode3 = __toESM(require("vscode"));
init_client();
function showLoginWebview(extensionUri, auth2) {
    ...
    try {
      if (msg.type === "oauth") {
        ...
        vscode3.env.openExternal(
          vscode3.Uri.parse(`${serverUrl}/auth/${msg.provider}`)
        );
      ...
      if (msg.type === "login") {
        const u = await auth2.login(msg.email, msg.password);
        vscode3.window.showInformationMessage(
          `Welcome back, ${u.displayName || u.email}! \u{1F389}`
        );
        panel.dispose();
      } else if (msg.type === "register") {
        const u = await auth2.register(msg.email, msg.password, msg.displayName);
        vscode3.window.showInformationMessage(
          `Account created! Welcome, ${u.displayName || u.email}! \u{1F389}`
        );
        panel.dispose();
      }
    } catch (err) {
  ...
  const disposable = auth2.onDidChange((user) => {
    if (user) {
      try {
        panel.dispose();
      } catch {
...
<button class="btn-oauth btn-google" onclick="doOAuth('google')" id="g-btn">\u{1F535} Sign in with Google</button>
...
// src/sidebar/DashboardProvider.ts
...
init_client();
var unwrap2 = (r) => r.data?.data ?? r.data;
var documentsApi = {
  list: async (projectId) => unwrap2(await getApi().get(`/api/projects/${projectId}/documents`)),
  getByType: async (projectId, type) => unwrap2(await getApi().get(`/api/projects/${projectId}/documents/${type}`)),
  upsert: async (projectId, type, data) => unwrap2(await getApi().put(`/api/projects/${projectId}/documents/${type}`, data)),
  delete: async (projectId, type) => {
...
init_client();
var unwrap3 = (r) => r.data?.data ?? r.data;
var sessionsApi = {
  list: async (projectId) => unwrap3(await getApi().get(`/api/projects/${projectId}/sessions`)),
  getById: async (projectId, id) => unwrap3(await getApi().get(`/api/projects/${projectId}/sessions/${id}`)),
  run: async (projectId, data) => unwrap3(await getApi().post(`/api/projects/${projectId}/sessions/run`, data))
};
...
var DashboardProvider = class {
  ...
  static {
    this.viewType = "uigenai.dashboard";
  }
  ...
  async _handleMessage(msg) {
    try {
      switch (msg.cmd) {
        case "login":
          vscode10.commands.executeCommand("uigenai.login");
          break;
        case "logout":
          vscode10.commands.executeCommand("uigenai.logout");
          break;
        case "oauthGoogle":
          vscode10.env.openExternal(
            vscode10.Uri.parse(`${getServerUrl()}/auth/google`)
          );
          break;
        case "oauthGithub":
          vscode10.env.openExternal(
            vscode10.Uri.parse(`${getServerUrl()}/auth/github`)
          );
          break;
        case "loadProjects":
          ...
          break;
        case "createProject":
          vscode10.commands.executeCommand("uigenai.createProject");
          break;
        case "editProject":
          {
            const { editProjectCmd: editProjectCmd2 } = await Promise.resolve().then(() => (init_projectCommands(), projectCommands_exports));
            await editProjectCmd2(msg.id);
          }
          break;
        case "deleteProject":
          {
            const { deleteProjectCmd: deleteProjectCmd2 } = await Promise.resolve().then(() => (init_projectCommands(), projectCommands_exports));
            await deleteProjectCmd2(msg.id, msg.name);
          }
          break;
        case "loadDocuments":
        ...
        case "viewDocument": {
          ...
          const td = await vscode10.workspace.openTextDocument({
            content: doc.content,
            language: "json"
          });
          await vscode10.window.showTextDocument(td, { preview: true });
          break;
        }
        case "deleteDocument":
          if (await confirmDelete("this document")) {
            await documentsApi.delete(msg.projectId, msg.type);
            vscode10.window.showInformationMessage("Document deleted.");
            this._post("documentsDeleted", {
              projectId: msg.projectId,
              type: msg.type
            });
            vscode10.commands.executeCommand("uigenai.refreshSidebar");
          }
          break;
        case "loadSessions":
          this._post("sessions", {
          ...
          break;
        case "runSession":
          vscode10.commands.executeCommand("uigenai.runSession", msg.projectId);
          break;
        case "viewSession": {
          const s = await sessionsApi.getById(msg.projectId, msg.id);
          const td = await vscode10.workspace.openTextDocument({
            content: s.output_summary_md || `Status: ${s.status}
Provider: ${s.provider}
Model: ${s.model}`,
            language: "markdown"
          });
        ...
        case "loadApis":
          ...
          break;
        case "createApi":
          vscode10.commands.executeCommand("uigenai.createApi");
          break;
        case "editApi":
          {
            const { editApiCmd: editApiCmd2 } = await Promise.resolve().then(() => (init_apiCommands(), apiCommands_exports));
            await editApiCmd2(msg.id);
          }
          break;
        case "deleteApi":
          {
            const { deleteApiCmd: deleteApiCmd2 } = await Promise.resolve().then(() => (init_apiCommands(), apiCommands_exports));
            await deleteApiCmd2(msg.id, msg.name);
          }
          break;
        case "loadConfigs":
          this._post("configs", {
        ...
        case "createConfig":
          {
            const { createApiConfigCmd: createApiConfigCmd2 } = await Promise.resolve().then(() => (init_apiConfigCommands(), apiConfigCommands_exports));
            await createApiConfigCmd2(msg.apiId);
          }
          break;
        case "editConfig":
          {
            const { editApiConfigCmd: editApiConfigCmd2 } = await Promise.resolve().then(() => (init_apiConfigCommands(), apiConfigCommands_exports));
            await editApiConfigCmd2(msg.apiId, msg.id);
          }
          break;
        case "deleteConfig":
          {
            const { deleteApiConfigCmd: deleteApiConfigCmd2 } = await Promise.resolve().then(() => (init_apiConfigCommands(), apiConfigCommands_exports));
            await deleteApiConfigCmd2(msg.apiId, msg.id);
          }
          break;
        case "loadSchemas":
          this._post("schemas", {
        ...
        case "createSchema":
          {
            const { createUiSchemaCmd: createUiSchemaCmd2 } = await Promise.resolve().then(() => (init_uiSchemaCommands(), uiSchemaCommands_exports));
            await createUiSchemaCmd2(msg.apiId);
          }
          break;
        case "editSchema":
          {
            const { editUiSchemaCmd: editUiSchemaCmd2 } = await Promise.resolve().then(() => (init_uiSchemaCommands(), uiSchemaCommands_exports));
            await editUiSchemaCmd2(msg.apiId, msg.id);
          }
          break;
        case "deleteSchema":
          {
            const { deleteUiSchemaCmd: deleteUiSchemaCmd2 } = await Promise.resolve().then(() => (init_uiSchemaCommands(), uiSchemaCommands_exports));
            await deleteUiSchemaCmd2(msg.apiId, msg.id);
          }
          break;
        case "loadCodes":
          this._post("codes", {
        ...
        case "viewCode":
          {
            const { viewGeneratedCodeCmd: viewGeneratedCodeCmd2 } = await Promise.resolve().then(() => (init_generatedCodeCommands(), generatedCodeCommands_exports));
            await viewGeneratedCodeCmd2(msg.apiId, msg.id);
          }
          break;
        case "applyCode":
          {
            const { applyGeneratedCodeCmd: applyGeneratedCodeCmd2 } = await Promise.resolve().then(() => (init_generatedCodeCommands(), generatedCodeCommands_exports));
            await applyGeneratedCodeCmd2(msg.apiId, msg.id);
          }
          break;
        case "deleteCode":
          {
            const { deleteGeneratedCodeCmd: deleteGeneratedCodeCmd2 } = await Promise.resolve().then(() => (init_generatedCodeCommands(), generatedCodeCommands_exports));
            await deleteGeneratedCodeCmd2(msg.apiId, msg.id);
          }
          break;
        ...
        case "createDeployment":
          vscode10.commands.executeCommand("uigenai.createDeployment");
          break;
        case "updateDeployment":
          {
            const { updateDeploymentStatusCmd: updateDeploymentStatusCmd2 } = await Promise.resolve().then(() => (init_deploymentCommands(), deploymentCommands_exports));
            await updateDeploymentStatusCmd2(msg.apiId, msg.id);
          }
          break;
        case "deleteDeployment":
          if (await confirmDelete("this deployment")) {
            await deploymentsApi.delete(msg.apiId, msg.id);
            vscode10.window.showInformationMessage("Deployment deleted.");
            vscode10.commands.executeCommand("uigenai.refreshSidebar");
          }
          break;
        case "generate":
          vscode10.commands.executeCommand("uigenai.generate");
          break;
      }
    } catch (e) {
      const m = e.response?.data?.error?.message || e.message || "Unknown error";
      vscode10.window.showErrorMessage(m);
    }
  }
  _post(type, data) {
...
` : `
  ...
  <div style="display:flex;gap:4px;flex-wrap:wrap">
    ...
    <button class="btn-s" onclick="send('oauthGoogle')" title="Google">\u{1F535} Google</button>
  ...
  c.innerHTML = projects.map(p => \`
    <div class="item">
      ...
      <div class="item-actions">
        ...
        <button class="btn-icon" title="Delete" onclick="send('deleteProject',{id:'\${p.id}',name:'\${esc(p.name)}'})">\u{1F5D1}\uFE0F</button>
      </div>
    </div>
    <div class="sub" id="psub-\${p.id}" style="display:none">
      <div class="sub-hd" onclick="toggleProjectSubSec('\${p.id}','docs')">
        \u{1F4C4} Documents <button class="btn-icon" onclick="event.stopPropagation();send('uploadDocument',{projectId:'\${p.id}'})">\uFF0B</button>
      </div>
      <div class="sub-body" id="pdocs-\${p.id}"></div>
      <div class="sub-hd" onclick="toggleProjectSubSec('\${p.id}','sessions')">
        \u{1F504} Sessions <button class="btn-icon" onclick="event.stopPropagation();send('runSession',{projectId:'\${p.id}'})">\u25B6</button>
      </div>
...
function renderDocuments(projectId, docs) {
  ...
  c.innerHTML = docs.map(d => \`
    <div class="item">
      ...
      <div class="item-actions">
        <button class="btn-icon" title="Delete" onclick="send('deleteDocument',{projectId:'\${projectId}',type:'\${d.type}'})">\u{1F5D1}\uFE0F</button>
      </div>
    </div>
  \`).join('');
}
  ...
  c.innerHTML = apis.map(a => \`
    <div class="item">
      ...
      <div class="item-actions">
        ...
        <button class="btn-icon" title="Delete" onclick="send('deleteApi',{id:'\${a.id}',name:'\${esc(a.name)}'})">\u{1F5D1}\uFE0F</button>
      </div>
    </div>
    <div class="sub" id="asub-\${a.id}" style="display:none">
      ...
      <div class="sub-hd" onclick="toggleApiSubSec('\${a.id}','schemas')">
        \u{1F4D0} UI Schemas <button class="btn-icon" onclick="event.stopPropagation();send('createSchema',{apiId:'\${a.id}'})">\uFF0B</button>
      </div>
      <div class="sub-body" id="asch-\${a.id}"></div>

      <div class="sub-hd" onclick="toggleApiSubSec('\${a.id}','codes')">
        \u{1F4BB} Generated Codes
      </div>
      <div class="sub-body" id="acod-\${a.id}"></div>

      <div class="sub-hd" onclick="toggleApiSubSec('\${a.id}','deployments')">
        \u{1F680} Deployments <button class="btn-icon" onclick="event.stopPropagation();send('createDeployment',{apiId:'\${a.id}'})">\uFF0B</button>
      </div>
  ...
  c.innerHTML = configs.map(cfg => \`
    <div class="item">
      ...
      <div class="item-actions">
        ...
        <button class="btn-icon" title="Delete" onclick="send('deleteConfig',{apiId:'\${apiId}',id:'\${cfg.id}'})">\u{1F5D1}\uFE0F</button>
      </div>
    </div>
  \`).join('');
}
  ...
  c.innerHTML = schemas.map(s => \`
    <div class="item">
      ...
      <div class="item-actions">
        ...
        <button class="btn-icon" title="Delete" onclick="send('deleteSchema',{apiId:'\${apiId}',id:'\${s.id}'})">\u{1F5D1}\uFE0F</button>
      </div>
    </div>
  \`).join('');
}
...
function renderCodes(apiId, codes) {
  ...
  c.innerHTML = codes.map(code => \`
    <div class="item">
      ...
      <div class="item-actions">
        <button class="btn-icon" title="Apply to workspace" onclick="send('applyCode',{apiId:'\${apiId}',id:'\${code.id}'})">\u{1F4C1}</button>
        <button class="btn-icon" title="Delete" onclick="send('deleteCode',{apiId:'\${apiId}',id:'\${code.id}'})">\u{1F5D1}\uFE0F</button>
      </div>
    </div>
  \`).join('');
}
  ...
  c.innerHTML = deployments.map(d => \`
    <div class="item">
      ...
      <div class="item-actions">
        ...
        <button class="btn-icon" title="Delete" onclick="send('deleteDeployment',{apiId:'\${apiId}',id:'\${d.id}'})">\u{1F5D1}\uFE0F</button>
      </div>
    </div>
  \`).join('');
}
...
async function confirmDelete(thing) {
  const r = await vscode10.window.showWarningMessage(
    `Delete ${thing}?`,
    { modal: true },
    "Delete"
  );
...
async function uploadDocumentCmd(projectId) {
  if (!projectId) {
    vscode11.window.showErrorMessage("Please select a project first.");
    return;
  }
  const typePick = await vscode11.window.showQuickPick(DOC_TYPES, { title: "Document Type", placeHolder: "Select document type" });
  if (!typePick) {
    return;
  }
  const source = await vscode11.window.showQuickPick(
    [{ label: "\u{1F4C4} From current editor", value: "editor" }, { label: "\u{1F4C1} From file", value: "file" }, { label: "\u270F\uFE0F Paste content", value: "paste" }],
  ...
  if (source.value === "editor") {
    ...
    const files = await vscode11.window.showOpenDialog({ canSelectMany: false, filters: { "All Files": ["*"] } });
    if (!files || files.length === 0) {
      return;
    }
    const buf = await vscode11.workspace.fs.readFile(files[0]);
    content = Buffer.from(buf).toString("utf-8");
    name = files[0].path.split("/").pop() || name;
  } else {
    const pasted = await vscode11.window.showInputBox({ title: "Paste content", prompt: "Paste document content (JSON/YAML)", ignoreFocusOut: true });
    if (!pasted) {
      return;
    }
    content = pasted;
  ...
  try {
    ...
    vscode11.window.showInformationMessage(`Document "${typePick.label}" uploaded!`);
    vscode11.commands.executeCommand("uigenai.refreshSidebar");
  } catch (e) {
    vscode11.window.showErrorMessage(`Failed: ${e.response?.data?.error?.message || e.message}`);
  }
}

// src/commands/sessionCommands.ts
var vscode12 = __toESM(require("vscode"));
async function runSessionCmd(projectId) {
  if (!projectId) {
    vscode12.window.showErrorMessage("Please select a project first.");
    return;
  }
  ...
  const model = await vscode12.window.showInputBox({
    title: "Model",
  ...
  const framework = await vscode12.window.showQuickPick(
    [{ label: "React", value: "react" }, { label: "Vue", value: "vue" }, { label: "Angular", value: "angular" }],
    { title: "Framework" }
  );
  const css = await vscode12.window.showQuickPick(
    [{ label: "Tailwind CSS", value: "tailwind" }, { label: "CSS Modules", value: "css-modules" }, { label: "Styled Components", value: "styled-components" }],
    { title: "CSS Strategy" }
  );
  await vscode12.window.withProgress(
    { location: vscode12.ProgressLocation.Notification, title: "Running generation session..." },
    async () => {
      try {
        ...
        vscode12.window.showInformationMessage(`Session started! Status: ${session.status}`);
        vscode12.commands.executeCommand("uigenai.refreshSidebar");
      } catch (e) {
        vscode12.window.showErrorMessage(`Failed: ${e.response?.data?.error?.message || e.message}`);
      }
    }
  );
}
...
init_client();
var generateApi = {
  generate: async (data) => (await getApi().post("/api/generate", data)).data
};
...
async function generateCmd() {
  ...
  const cfg = vscode13.workspace.getConfiguration("uigenai");
  ...
  const model = await vscode13.window.showInputBox({
    title: "Model",
    value: provider.value === "gemini" ? cfg.get("defaultModel", "gemini-2.0-flash") : "gpt-4o"
  });
  if (!model) {
    return;
  }
  let apiId;
  const linkApi = await vscode13.window.showQuickPick(
  ...
  if (linkApi?.value === "yes") {
    try {
      ...
      if (apis.length > 0) {
        ...
        vscode13.window.showWarningMessage("No APIs found. Generating without saving.");
      }
    } catch {
    }
  }
  ...
  await vscode13.window.withProgress(
    { location: vscode13.ProgressLocation.Notification, title: "\u{1F916} Generating code..." },
    async () => {
      try {
        result = await generateApi.generate({ prompt: prompt.trim(), provider: provider.value, model, apiId });
      } catch (e) {
        vscode13.window.showErrorMessage(`Generation failed: ${e.response?.data?.error?.message || e.message}`);
      }
    }
  );
  if (!result?.success || !result.changes.length) {
    if (result?.changes?.length === 0) {
      vscode13.window.showWarningMessage("AI returned no code. Try a more specific prompt.");
    }
    return;
  }
  showPreview(result, prompt.trim());
}
function showPreview(result, prompt) {
  const panel = vscode13.window.createWebviewPanel("uigenai-preview", "\u26A1 Generated Code", vscode13.ViewColumn.One, { enableScripts: true });
  panel.webview.onDidReceiveMessage(async (msg) => {
    ...
    if (msg.type === "copy") {
      await vscode13.env.clipboard.writeText(result.changes[msg.i].codeContent);
      vscode13.window.showInformationMessage("Copied!");
    }
  });
...
<button class="btn bp" onclick="applyAll()" id="ab">\u{1F4C1} Apply All</button></div>
<div class="prompt">\u{1F4AC} ${esc2(prompt)}</div></div>
${result.summary ? `<div class="sm">${esc2(result.summary)}</div>` : ""}
...
files.forEach((f,i)=>{const d=document.createElement('div');d.className='fi';d.innerHTML=\`<div class="fh" onclick="tog(\${i})"><div class="fn">\u{1F4C4} \${esc(f.name)}</div><div style="display:flex;align-items:center;gap:10px"><div class="fm">\${f.lang} \xB7 \${f.lines} lines</div><div class="fa"><button class="btn bs" style="padding:3px 8px;font-size:10px" onclick="event.stopPropagation();cp(\${i})">\u{1F4CB}</button><button class="btn bs" style="padding:3px 8px;font-size:10px" onclick="event.stopPropagation();af(\${i})">\u{1F4C1}</button></div></div></div><div class="cb" id="c\${i}"><pre>\${esc(f.content)}</pre></div>\`;fl.appendChild(d)});
...
async function applyFiles(changes) {
  const wf = vscode13.workspace.workspaceFolders;
  let base;
  if (!wf?.length) {
    const f = await vscode13.window.showOpenDialog({ canSelectFolders: true, canSelectFiles: false, title: "Select target folder" });
    if (!f?.length) {
      return;
    }
    base = f[0];
  } else {
    base = wf.length === 1 ? wf[0].uri : (await vscode13.window.showWorkspaceFolderPick())?.uri || wf[0].uri;
  }
  let n = 0;
  for (const c of changes) {
    ...
    const uri = vscode13.Uri.joinPath(base, p);
    try {
      await vscode13.workspace.fs.createDirectory(vscode13.Uri.joinPath(uri, ".."));
    } catch {
    }
    await vscode13.workspace.fs.writeFile(uri, Buffer.from(c.codeContent, "utf-8"));
    n++;
  }
  vscode13.window.showInformationMessage(`\u2705 ${n} file(s) written!`);
  if (changes[0]) {
    try {
      const doc = await vscode13.workspace.openTextDocument(vscode13.Uri.joinPath(base, changes[0].fileName.replace(/^\/+/, "")));
      ...
      if (accessToken && refreshToken) {
        try {
          ...
          vscode14.window.showInformationMessage(
            `Welcome, ${user.displayName || user.email}! \u{1F389}`
          );
        } catch (e) {
          vscode14.window.showErrorMessage(`OAuth login failed: ${e.message}`);
        }
      } else {
        vscode14.window.showErrorMessage("OAuth callback missing tokens.");
      }
    }
  }
};
async function activate(context) {
  ...
  context.subscriptions.push(
    vscode14.window.registerUriHandler(new OAuthCallbackHandler(auth))
  );
  ...
  const reg = (id, fn) => context.subscriptions.push(vscode14.commands.registerCommand(id, fn));
  reg("uigenai.login", () => showLoginWebview(context.extensionUri, auth));
  reg("uigenai.logout", async () => {
    await auth.logout();
    vscode14.window.showInformationMessage("Logged out.");
  });
  reg("uigenai.generate", () => generateCmd());
  reg("uigenai.setServer", async () => {
    const current = vscode14.workspace.getConfiguration("uigenai").get("serverUrl", "http://localhost:3000");
    const url2 = await vscode14.window.showInputBox({
      title: "Server URL",
      value: current,
      prompt: "Backend API URL"
    });
    if (url2 !== void 0) {
      await vscode14.workspace.getConfiguration("uigenai").update("serverUrl", url2, vscode14.ConfigurationTarget.Global);
      initApiClient({
        getToken: async () => context.secrets.get("uigenai.accessToken"),
        onAuthFailed: () => auth.logout()
      });
      dashboardProvider.refresh();
      vscode14.window.showInformationMessage(`Server URL set to ${url2}`);
    }
  });
  ...
  reg(
    "uigenai.uploadDocument",
    (projectId) => uploadDocumentCmd(projectId)
  );
  reg("uigenai.runSession", (projectId) => runSessionCmd(projectId));
