...
export async function runSessionCmd(projectId?: string) {
  if (!projectId) { vscode.window.showErrorMessage("Please select a project first."); return; }
  ...
  await vscode.window.withProgress(
    { location: vscode.ProgressLocation.Notification, title: "Running generation session..." },
    async () => {
      try {
        ...
        vscode.window.showInformationMessage(`Session started! Status: ${session.status}`);
