...
export interface ApiConfig {
  id: string; api_id: string; key: string; value: string; is_secret: boolean;
...
export const apiConfigsApi = {
  ...
  create: async (apiId: string, data: { key: string; value: string; is_secret?: boolean }): Promise<ApiConfig> =>
    unwrap(await getApi().post(`/api/apis/${apiId}/configs`, data)),
  update: async (apiId: string, id: string, data: Partial<ApiConfig>): Promise<ApiConfig> =>
