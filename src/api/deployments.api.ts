...
export const deploymentsApi = {
  ...
  create: async (apiId: string, data: { environment?: string; status?: string; provider?: string; metadata_json?: any }): Promise<Deployment> =>
    unwrap(await getApi().post(`/api/apis/${apiId}/deployments`, data)),
  update: async (apiId: string, id: string, data: Partial<Deployment>): Promise<Deployment> =>
